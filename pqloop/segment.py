...
def final_encode(ff, space, params, cfg, source, out_dir, fmt="hls",
    ...
    if fmt == "hls":
        ...
        args += ["-f", "dash", "-seg_duration", f"{seg:g}",
                 "-use_template", "1", "-use_timeline", "1", str(manifest)]
        main_output = manifest
    elif fmt == "mp4":
        outfile = out_dir / "output.mp4"
        args += ["-movflags", "+faststart", "-f", "mp4", str(outfile)]
        main_output = outfile
    else:
        raise ValueError(f"unknown output format {fmt!r} (hls, dash, mp4)")

    log(f"encoding deliverable -> {main_output} "
        f"({'deinterlaced ' if deinterlaced else ''}{fmt}, "
        f"{target}k, GOP {gop_len} @ {fps:g}fps, seg {seg:g}s)")
    ff.run(args, timeout=None)
    ...
    return {"output": str(main_output), "segments": len(segments),
            "gop": gop_len, "fps": fps, "deinterlaced": deinterlaced}
